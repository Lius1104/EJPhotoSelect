...
@interface EJZoomingScrollView () {
    
    EJPhotoBrowser __weak *_photoBrowser;
	EJTapDetectingView *_tapView; // for background taps
	EJTapDetectingImageView *_photoImageView;
...
@implementation EJZoomingScrollView
    ...
    if ((self = [super init])) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(setProgressFromNotification:)
                                                     name:EJPHOTO_PROGRESS_NOTIFICATION
                                                   object:nil];
...
- (void)configPhoto:(id<EJPhoto>)photo thumb:(id<EJPhoto>)thumb {
    ...
    _thumb = thumb;
    UIImage * img;
    if (_photo.isVideo == NO) {// 不是视频
        img = [_photoBrowser imageForPhoto:_photo];
    } else {// 视频
        EJPhoto * item = (EJPhoto *)_photo;
        if ([item.videoURL.absoluteString length] > 0) {// 网络视频
            img = [_photoBrowser imageForPhoto:_thumb];
        } else {
            img = [_photoBrowser imageForPhoto:_photo];
        }
    }
...
//- (void)setPhoto:(id<EJPhoto>)photo {
...
- (void)displayImage {
	if ((_photo || _thumb) && _photoImageView.image == nil) {
...
- (void)displayImageFailure {
    ...
    if (![_photo respondsToSelector:@selector(emptyImage)] || !_photo.emptyImage) {
        if (!_loadingError) {
            ...
            _loadingError.image = [UIImage imageForResourcePath:@"EJPhotoBrowser.bundle/ImageError" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]];
...
- (void)setProgressFromNotification:(NSNotification *)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        id <EJPhoto> photoWithProgress = [dict objectForKey:@"photo"];
